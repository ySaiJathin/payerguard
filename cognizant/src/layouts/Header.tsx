...
import { mockNotifications } from '../data/mockMetrics';
    ...
    <header className="h-16 sticky top-0 z-30 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 sm:px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3 sm:gap-4 flex-1 max-w-xl">
        ...
        <form onSubmit={handleSearchSubmit} className="relative w-full max-w-md">
          ...
          <input
            ...
            placeholder="Search Claim #, Patient ID, NPI, or INC-8921..."
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="hidden md:flex items-center gap-3 px-3 py-1.5 rounded-lg bg-slate-950/60 border border-slate-800 text-xs">
          <StatusIndicator status="operational" label="HIPAA Gateway Live" size="sm" />
          <span className="text-slate-700">|</span>
          <div className="flex items-center gap-1.5 text-slate-400 text-[11px] font-mono">
            <Database className="w-3 h-3 text-cyan-400" />
            <span>EDI 5010 Engine</span>
          </div>
        </div>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="w-7 h-7 rounded-lg bg-linear-to-tr from-cyan-600 to-indigo-600 flex items-center justify-center text-xs font-bold text-white border border-cyan-400/40">
              SJ
            </div>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-56 rounded-xl bg-slate-900 border border-slate-750 shadow-2xl z-50 p-2 text-slate-200 animate-in fade-in zoom-in-95 duration-100">
              <div className="p-2.5 border-b border-slate-800 mb-1">
                <p className="text-xs font-semibold text-white">Dr. Sarah Jenkins</p>
                <p className="text-[11px] text-slate-400 font-mono">sarah.jenkins@payerguard.internal</p>
                <span className="mt-1.5 inline-block text-[10px] px-1.5 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-800/40 font-mono">
                  Role: Auditor / Admin
                </span>
              </div>
