...
- **FR-008**: System MUST include a freshness expectation on batch-level recency (e.g., that a batch's data falls within the expected ingestion window), scoped to what batch ingestion actually provides (the continuous-ingestion phase was removed 2026-08-18) — not fabricated from a field that doesn't represent true processing freshness (see MVP_CONTEXT.md Section 2.4 on `NCH_WKLY_PROC_DT` not being a real operational timestamp).
